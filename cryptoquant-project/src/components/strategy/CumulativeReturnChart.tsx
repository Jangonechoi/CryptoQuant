...
export default function CumulativeReturnChart({
  ...
  useEffect(() => {
    ...
    // 데이터가 없으면 빈 배열로 설정
    if (!cumulativeReturnCurve || cumulativeReturnCurve.length === 0) {
      try {
        seriesRef.current.setData([]);
      } catch (error) {
        console.error("Error clearing cumulative return data:", error);
      }
      return;
    }

    try {
      const formattedData = cumulativeReturnCurve.map((item) => ({
        time: item.time,
        value: item.value,
      }));
      ...
      console.error("Error setting cumulative return data:", error);
    }
  }, [cumulativeReturnCurve, isChartReady]);
  ...
  if (!cumulativeReturnCurve || cumulativeReturnCurve.length === 0) {
    return (
      <Card className="h-full">
        <div className="h-[400px] flex items-center justify-center">
          <div className="text-neutral-400">누적 수익률 데이터가 없습니다.</div>
        </div>
      </Card>
    );
  }
